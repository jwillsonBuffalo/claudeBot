...
class TradeMonitor:
    ...
    def get_new_trades(self) -> list:
        ...
        for trade in raw:
            ...
            if self.is_tracked_market(trade):
...
class ChainMonitor:
    ...
    def get_new_trades(self) -> list:
        ...
        for log in logs:
            ...
            # "Pass through unknown" was causing untracked/expired markets to be copied.
            if not trade["slug"]:
                unknown_count += 1
    ...
    def _maybe_refresh_metadata(self) -> None:
        ...
        for pattern in self.config.market_slug_patterns:
...
class TradeExecutor:
    ...
    def place_order(
        ...
        """
        Place a FAK order at the current live ask price.

        BUY:  fetch live ask → spend same dollar amount as target (size × ref_price)
              → one FAK attempt; if book empty, wait 2 s and retry once.
        SELL: one FAK at ref_price × (1 - slippage_pct).

        Raises RuntimeError if the CLOB client is not ready or the order fails.
        """
        if not self.client:
        ...
        tick_str  = self._get_tick_size(token_id)
        tick      = float(tick_str)
        ...
        options = CreateOrderOptions(tick_size=tick_str, neg_risk=neg_risk)
        is_buy = order_side == BUY

        if is_buy:
            # Spend the same dollar amount as the target, at whatever the
            # live ask is right now (not the stale ref_price).
            budget_usd = float(size) * ref_price

            def _fetch_ask() -> float:
                try:
                    r = self.client.get_price(token_id, "buy")
                    return float(r.get("price", 0) or 0)
                except Exception:
                    return 0.0

            def _build_order(ask: float):
                # API constraint: maker_amount (shares × price) must have ≤ 2 dp.
                # Use integer arithmetic: step = 100 / gcd(price_hundredths, 100)
                # and take the largest valid integer share count ≤ floor(shares × 100).
                use_price = ask if ask > 0 else ref_price
                order_price = min(_round_to_tick(use_price, tick, round_up=True), 0.99)
                pd = D(str(order_price))
                shares_f = budget_usd / float(pd) if float(pd) > 0 else float(size)
                p_int = round(float(pd) * 100)
                s_max_int = math.floor(shares_f * 100)
                gcd_val = math.gcd(p_int, 100) if p_int > 0 else 1
                step = 100 // gcd_val
                s_int = (s_max_int // step) * step
                if s_int <= 0:
                    raise RuntimeError(
                        f"BUY size too small: budget=${budget_usd:.4f} price={order_price:.4f}"
                    )
                sd = D(s_int) / D(100)
                args = OrderArgs(token_id=token_id, price=float(pd), size=float(sd), side=order_side)
                return args, sd

            live_ask = _fetch_ask()
            order_args, size_d = _build_order(live_ask)
            try:
            ...
            try:
                ...
                response_d = resp if isinstance(resp, dict) else {"raw": str(resp)}
            except Exception as exc:
                err_l = str(exc).lower()
                is_empty = (
                ...
                if not is_empty:
                    raise
                # Book momentarily empty — wait 2 s, re-fetch ask, one more try
                time.sleep(2)
                live_ask2 = _fetch_ask()
                order_args2, size_d = _build_order(live_ask2)
                try:
                    signed2 = self.client.create_order(order_args2, options)
                except TypeError:
                    signed2 = self.client.create_order(order_args2)
                try:
                    resp2 = self.client.post_order(signed2, OrderType.FAK)
                    response_d = resp2 if isinstance(resp2, dict) else {"raw": str(resp2)}
                except Exception as exc2:
                    raise RuntimeError(f"BUY FAK failed (book still empty?): {exc2}") from exc2

            filled = self._extract_filled_shares(response_d, is_buy=True)
            if filled <= 0 and response_d.get("success") and str(
                response_d.get("status", "")
            ).lower() in ("matched", "filled"):
                filled = float(size_d)

            return {
                "success": True,
                "mode": "FAK_LIVE_ASK",
                "requested_shares": float(size_d),
                "filled_shares": max(0.0, filled),
            ...
            # SELL: one FAK at ref_price minus a small slippage buffer
            slip = max(0.0, float(getattr(self.config, "slippage_pct", 0.01)))
            order_price = max(_round_to_tick(ref_price * (1 - slip), tick, round_up=False), 0.01)
            price_d = D(str(order_price))
            size_d = D(str(size)).quantize(D("0.0001"), rounding=ROUND_DOWN).quantize(
                D("0.01"), rounding=ROUND_DOWN
            )
            if size_d <= 0:
                raise RuntimeError(f"SELL size too small after rounding: {size}")
            order_args = OrderArgs(
                token_id=token_id, price=float(price_d), size=float(size_d), side=order_side
            )
            try:
                signed = self.client.create_order(order_args, options)
            except TypeError:
            ...
            try:
                ...
                raise RuntimeError(f"SELL FAK failed: {exc}") from exc

            filled = self._extract_filled_shares(response_d, is_buy=False)
            if filled <= 0 and response_d.get("success") and str(
                response_d.get("status", "")
            ).lower() in ("matched", "filled"):
                filled = float(size_d)
...
class CopyTradingBot:
    ...
    def _process_trade(self, trade: dict) -> None:
        ...
        # ── Live execution ────────────────────────────────────────────────────
        try:
            response = self.executor.place_order(token_id, side, copy_size, their_price)
