...
class MarketAnalyzer:
    """
    Thread-safe, lazily-refreshed BTC 5m market predictor.
    ...
    def _fetch_polymarket_chainlink(self, limit: int = 20, tf_cfg: Optional[dict] = None) -> List[dict]:
        ...
        r = self._session.get(
            ...
            params={"symbol": "BTC", "interval": tf_cfg.get("candle_interval", "5m")},
    ...
    def _fetch_kraken_1m(self, limit: int = 20) -> List[dict]:
        ...
        is unavailable. Columns: [time, open, high, low, close, vwap, vol, count]."""
    ...
    def _fetch_chainlink(self, limit: int = 20, tf_cfg: Optional[dict] = None) -> List[dict]:
        """
        Read Chainlink BTC/USD feed on Polygon via batch JSON-RPC eth_call.
    ...
    def _chainlink_via_rpc(self, rpc_url: str, limit: int) -> List[dict]:
        ...
        raw = self._rpc_single(rpc_url, "eth_call", [
            {"to": _CL_BTC_USD, "data": _LATEST_SEL}, "latest"
        ])
        ...
        for i in range(1, _CL_BATCH_ROUNDS + 1):
            ...
            batch.append({
                "jsonrpc": "2.0",
                "method":  "eth_call",
                "params":  [{"to": _CL_BTC_USD, "data": data}, "latest"],
                "id":      i,
            })
    ...
    def _fetch_bybit(self, limit: int = 20, tf_cfg: Optional[dict] = None) -> List[dict]:
        ...
        url = (
            ...
            f"?category=spot&symbol=BTCUSDT&interval={tf_cfg.get('candle_interval_bybit', '5')}&limit={limit}"
        )
        resp = self._session.get(url, timeout=FETCH_TIMEOUT_SECONDS)
        resp.raise_for_status()
        data = resp.json()
    ...
    def _fetch_binance_us(self, limit: int = 20, tf_cfg: Optional[dict] = None) -> List[dict]:
        ...
        url = (
            ...
            f"?symbol=BTCUSDT&interval={tf_cfg.get('candle_interval', '5m')}&limit={limit}"
        )
        resp = self._session.get(url, timeout=FETCH_TIMEOUT_SECONDS)
        resp.raise_for_status()
